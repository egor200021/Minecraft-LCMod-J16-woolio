...
void StatePlay::ChangeInvList(short x)
{
    ...
    for(int i = 0; i!=9; i++)
    {
    if ((( i+((j)*9)) + 1 + (x*45)) > allcubes-10)
    {
    invBar[j][i] = 0;
    }
    else
    {
    invBar[j][i] = ( i+((j)*9)) + 1 + (x*45);
    }
    }
    }
...
void StatePlay::LoadTextures()
{
    ...
    invSprite = new Sprite(TextureHelper::Instance()->GetTexture(TextureHelper::inv),0,0,182,102);
    invSprite->SetPosition(240,100);
    invSprite->Scale(1.75f,1.75f);

    selectInvSprite = new Sprite(TextureHelper::Instance()->GetTexture(TextureHelper::inv),187,0,22,22);
    selectInvSprite->SetPosition(100,20);
    selectInvSprite->Scale(1.75f,1.75f);
    ...
    }else
    {
	    ...
	    skyClouds = new SkyClouds();
	    skyClouds->SetTexture(TextureHelper::Instance()->GetTexture(TextureHelper::clouds1));
	    skyClouds->UpdateClouds();
...
void StatePlay::HandleEvents(StateManager* sManager)
{
    ...
    if(menuState == 0)//game state
    {
                    ...
                    if(distance != -1.0f)
                    {
                        ...
                        if(mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) == Lava::getID() || mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) == 4 || mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) == 0 || mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) == Cloud::getID())
                        {
                            ...
                            if(!blockBox.intersect(playerBox))
                            {
                                ...
                                if(mWorld->CanPutBlockHere(testPos2.x,testPos2.y,testPos2.z,blockBar[barPosition]))
                                {
                                    if(mWorld->LightSourceBlock(blockBar[barPosition]))
                                    {
                                        mWorld->SetLigtSourcePosition(testPos2.x,testPos2.y,testPos2.z,blockBar[barPosition]);
                                    }

                                    if (blockBar[barPosition]!=74)
                                    {
                                    mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) = blockBar[barPosition];//set block typeint
                                    }
                                    else
                                    {
                                    short r = rand() % 4;
                                        ...
                                        switch(r)
                                        {
                                            case 0: mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) = 74; break;
                                            case 1: mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) = 121; break;
                                            case 2: mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) = 122; break;
                                            case 3: mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) = 123; break;
                                        }
                                    }

                                    if (blockBar[barPosition]==39)
                                    {
                                        for(int x1 = testPos2.x-3; x1 < testPos2.x+3; x1++)
                                        {
                                        for(int y1 = testPos2.y-3; y1 < testPos2.y+3; y1++)
                                        {
                                        for(int z1 = testPos2.z-3; z1 < testPos2.z+3; z1++)
                                        {
                                    ...
                                    if(chunkTarget != -1)
                                    {
                                        ...
                                        if(mWorld->LightSourceBlock(blockBar[barPosition]))
                                        {
                                            mWorld->RebuildChunksLight(testPos2,chunkTarget,(blockBar[barPosition]));
                                        }
                                        else
                                        {
            ...
            for(float i = 0; i < 5.25f; i+=0.25f)
            {
                ...
                if(mWorld->BlockEditable(testPos.x,testPos.y,testPos.z))
                {
                    ...
                    if (mWorld->GetBlock(testPos.x, testPos.y, testPos.z) == TNT::getID())
                    {
                        ...
                        for(int x = -3; x <= 3; x++)
                        {
                            for(int y = -3; y <= 3; y++)
                            {
                                for(int z = -3; z <= 3; z++)
                                {
                                            ...
                                            if (mWorld->GetBlock(x+testPos.x,y+testPos.y,z+testPos.z) == TNT::getID())
                                            {
                                                tntNum += 1;
                                            }
                                }
                        ...
                        for(int x = -tntNum; x <= tntNum; x++)
                        {
                            for(int y = -tntNum; y <= tntNum; y++)
                            {
                                for(int z = -tntNum; z <= tntNum; z++)
                                {
                                if(mWorld->GetBlock(x+testPos.x,y+testPos.y,+testPos.z) != IronBlock::getID() || mWorld->GetBlock(x+testPos.x,y+testPos.y,+testPos.z) != WaterBlock::getID())
                                {
                                if(sqrt(pow(sqrt(pow(x,2)+pow(y,2)),2)+pow(z,2))<=tntNum)
                                    {
        ...
        if(keyPressed(InputHelper::Instance()->getButtonToAction(9)))
        {
            ...
            selectInvSprite->SetPosition(100 + (invXPosition * 35),28+(invYPosition * 35));
        }
        ...
        {
            invYPosition != 4 ? invYPosition ++ : invYPosition = 4;
            selectInvSprite->SetPosition(100 + (invXPosition * 35),28+(invYPosition * 35));
        }
        ...
        {
            if (invBar[invYPosition][invXPosition] != 0)
            {
                blockBar[barPosition] = invBar[invYPosition][invXPosition];
            }
        }

        // right
        if(keyPressed(InputHelper::Instance()->getButtonToAction(7)))
        {
            barPosition != 8 ? barPosition ++ : barPosition = 0;
        ...
        // left
        if(keyPressed(InputHelper::Instance()->getButtonToAction(6)))
        {
            barPosition != 0 ? barPosition -- : barPosition = 8;
       ...
       if(isWalking && walkingOnGround && (!footInWater || !footInLava))
		{
			...
			{
                bobCycle += dt*2;
				if(bobCycle>360.0f){bobCycle=0.0f;}
...
void StatePlay::Draw(StateManager* sManager)
{
	...
	sceGuFog( 0.0f,(mWorld->fogLevel + 1) * 30.0f,GU_COLOR(mWorld->fogColor.x,mWorld->fogColor.y,mWorld->fogColor.z,1.0f));	// Fog parameters 100.75
	...
	//render cube in right hand
	{
		...
		float cubeBob1 = + cosf(bobCycle*180.0f*DEG_TO_RAD)*-0.1f+0.05f;
		ScePspFVector3 move = {0.47f+cubeBob,-0.32f+(cubeBob1/8),-0.7f};
		sceGumTranslate(&move);
		//rotate
		...
		sceGuDepthMask(1);
		mWorld->drawCubes((blockBar[barPosition]));
    ...
    else
    {
        ...
        mRender->DebugPrint(240,230,"Page %d / 3",page+1);
    }
    ...
    for(int k = 0; k != 9; k++)
	{
		...
		ScePspFVector3 loc = {100+k*35,29+l*35,0.0f};
		sceGumTranslate(&loc);
    ...
    if(menuOptions)
        {
            ...
            mWorld->freezeDayTime == true ? mRender->DebugPrint(240,105,"Day time freeze: ON"):mRender->DebugPrint(240,105,"Day time freeze: OFF");
